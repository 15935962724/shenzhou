...
<title>${message("admin.static.build")} - Powered By HaoKangHu</title>
...
$().ready(function() {
	...
	var $journalismCategoryTr = $("#journalismCategoryTr");
	var $journalismCategoryId = $("#journalismCategoryId");
	...
	$buildType.change(function() {
		...
		if ($this.val() == "article") {
			...
			$journalismCategoryTr.hide();
			$beginDateTr.show();
			$endDateTr.show();
		}else if ($this.val() == "journalism") {
			$journalismCategoryTr.show();
	...
	$inputForm.validate({
		...
		submitHandler: function(form) {
			...
			journalismCategoryId = $journalismCategoryId.val();
			...
			$journalismCategoryId.prop("disabled", true);
	...
	function build() {
		$.ajax({
			url: "build.jhtml",
			type: "POST",
			data: {buildType: buildType, articleCategoryId: articleCategoryId,journalismCategoryId: journalismCategoryId,  productCategoryId: productCategoryId, beginDate: beginDate, endDate: endDate, first: first, count: count},
			dataType: "json",
			cache: false,
			success: function(data) {
				...
				if (!data.isCompleted) {
					if (buildType == "article" || buildType == "product" || buildType == "journalism" ) {
						first = data.first;
					...
					$journalismCategoryId.prop("disabled", false);
...
<body>
	...
	<form id="inputForm" action="build.jhtml" method="post">
		<table class="input">
			...
			<tr id="journalismCategoryTr" class="hidden">
				<th>
					${message("Article.articleCategory")}:
				</th>
				<td>
					<select id="journalismCategoryId" name="journalismCategoryId">
						<option value="">${message("admin.static.emptyOption")}</option>
						[#list journalismCategoryTree as journalismCategory]
							<option value="${journalismCategory.id}">
