...
(function($) {
	...
	//机构上传logo wsr 2017-6-26 10:34:53
	$.fn.extend({
		uploadImg: function(options) {
			...
			var settings = {
				...
				isUpload: true,
				browserUrl: shenzhou.base + "/mechanism/mechanism/browser.jhtml",
				uploadUrl: shenzhou.base + "/mechanism/mechanism/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserUploadInput = $('<input type="file" name="file" \/> <input type="hidden" id="path" name="path" value="'+ options +'" \/>').appendTo($browserUploadButton);
					}
					...
					$browserList = $('<div class="browserList" style="display:none;"><\/div>').appendTo($browser);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = shenzhou.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = shenzhou.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
						...
						$browserList.find("img").bind("click", function() {
							...
							if (isDirectory == "true") {
								...
								if (settings.input != null) {
									...
									$browserButton.parents(".fieldSet").find(":text").val(url);
								}
						...
						if ($.trim(text) != "") {
							...
							if (data.message.type == "success") {
								if (settings.input != null) {
									...
									$browserButton.parents(".fieldSet").find(":text").val(data.url);
								}
								if (settings.callback != null && typeof settings.callback == "function") {
