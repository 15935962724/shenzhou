...
import net.shenzhou.template.method.AgeMethod;
 ...
 *
 */
@Controller("webAssessReportController")
@RequestMapping("/web/assessReport")
...
//	http://localhost:8080/shenzhou/app/assessReport/view.jhtml?file={assessReportId:"4"}
	...
	public void recoveryRecordList(String file,HttpServletRequest request ,HttpServletResponse response) throws IOException{
		...
		try {
			...
			Map<String ,Object> recoveryRecordlist = new HashMap<String, Object>();
			recoveryRecordlist.put("recoveryRecordlist", recoveryRecord_list);
			map.put("status", "200");
			map.put("message", "数据加载成功");
			map.put("data",JsonUtils.toJson(recoveryRecordlist));
			printWriter.write(JsonUtils.toString(map)) ;
			return;
	 ...
	 * web端报告列表
	 */
	@RequestMapping(value = "/toAssessReport", method = RequestMethod.GET)
	public String toAssessReport(ModelMap model,Long patientMemberId) {
		Map<String ,Object> map = new HashMap<String ,Object>();
			Member patientMember = memberService.find(patientMemberId);
			Map<String,Object> patientMember_data = new HashMap<String, Object>();
			patientMember_data.put("patientMemberId", patientMember.getId());
			...
			patientMember_data.put("logo", patientMember.getLogo());
			patientMember_data.put("gender", patientMember.getGender());
			patientMember_data.put("birth",AgeMethod.getAge(patientMember.getBirth()));
			patientMember_data.put("createDate", patientMember.getCreateDate());
			
			System.out.println(AgeMethod.getAge(patientMember.getBirth()));
			...
			if (lists.size()>0) {
				...
				if (recoveryPlanList.size()>0) {
					...
					assessReport_data_null.put("doctorName","");
					assessReport_data_null.put("mechanismName","");
					assessReport_data_null.put("createDate", DateUtil.getDatetoString("yyyy-MM-dd HH:mm:ss", new Date()));
					assessReport_list.add(assessReport_data_null);
				}
			}
			
			System.out.println(DateUtil.getDatetoString("yyyy-MM-dd HH:mm:ss", new Date()));
			...
			for (AssessReport assessReport : patientMember.getAssessReports()) {
				...
				if (assessReport!=null) {
					...
					assessReport_data.put("doctorName", assessReport.getDoctor().getName());
					assessReport_data.put("mechanismName",assessReport.getMechanism()==null?"":assessReport.getMechanism().getName());
					assessReport_data.put("createDate", assessReport.getCreateDate().toString());
					assessReport_list.add(assessReport_data);
				}
			}
			Collections.reverse(assessReport_list);
			model.addAttribute("patientMember", patientMember_data);
			model.addAttribute("assessReportList", assessReport_list);
			model.addAttribute("assessReportSize", assessReport_list.size());
			return "/web/report/assessReport";
	}
	 ...
	 * web端报告详情
	 */
	@RequestMapping(value = "/assessReportDetails", method = RequestMethod.GET,produces="text/html;charset=UTF-8")
	@ResponseBody
	public String assessReportDetails(ModelMap model,Long assessReportId,Long patientMemberId) {
	        AssessReport assessReport = assessReportService.find(assessReportId);//诊评报告
	        Member patientMember = memberService.find(patientMemberId);
			Map<String,Object> assessReport_data = new HashMap<String, Object>();
			assessReport_data.put("id", assessReport.getId());
			assessReport_data.put("diseaseName", assessReport.getDiseaseName());
			...
			assessReport_data.put("mechanismName", assessReport.getMechanism()==null?"":assessReport.getMechanism().getName());
			assessReport_data.put("doctorName", assessReport.getDoctor().getName());
			assessReport_data.put("proposal", assessReport.getProposal());
			assessReport_data.put("createDate", assessReport.getCreateDate());
			
			Date createDate = assessReport.getCreateDate();
			Date endDate = null;
			List lists = new ArrayList();
			for (AssessReport patientAssessReport : patientMember.getAssessReports()) {
				lists.add(patientAssessReport.getCreateDate());
			}
			ComparatorDate c = new ComparatorDate();
			Collections.sort(lists, c);  
			...
			for(int x = 0;x<lists.size();x++){
				...
				if(lists.get(x)==createDate){
					if(x==lists.size()-1){
						endDate=DateUtil.getStringtoDate("1970-01-01", "yyyy-MM-dd");
					}else{
						endDate=(Date)lists.get(x+1);
					}
				}
			}
	 ...
	 * web端计划详情
	 */
	@RequestMapping(value = "/recoveryPlanDetails", method = RequestMethod.GET,produces="text/html;charset=UTF-8")
	@ResponseBody
	public String recoveryPlanDetails(ModelMap model,Long recoveryPlanId,Long patientMemberId) {
			RecoveryPlan recoveryPlan = recoveryPlanService.find(recoveryPlanId);//诊评报告
	        Member patientMember = memberService.find(patientMemberId);
			Map<String,Object> recoveryPlan_map = new HashMap<String, Object>();
			List<Map<String ,Object>> drillContent_lsit = new ArrayList<Map<String ,Object>>();
			recoveryPlan_map.put("id", recoveryPlan.getId());//id
			...
			Date createDate = recoveryPlan.getCreateDate();
			Date endDate = null;
			List lists = new ArrayList();
			List<RecoveryPlan> list_ass = patientMember.getRecoveryPlans();
			for (RecoveryPlan recoveryPlans : patientMember.getRecoveryPlans()) {
				lists.add(recoveryPlans.getCreateDate());
			}
			ComparatorDate c = new ComparatorDate();
			Collections.sort(lists, c);  
			Collections.reverse(lists);
			...
			Map<String,Object> data_map = new HashMap<String, Object>();
			data_map.put("recoveryPlan", recoveryPlan_map);
			data_map.put("recoveryRecords",recoveryRecord_list);
