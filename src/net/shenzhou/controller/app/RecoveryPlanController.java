 ...
 *
 */
@Controller("appRecoveryPlanController")
@RequestMapping("/app/recoveryPlan")
public class RecoveryPlanController extends BaseController {
	...
	String view(Long id  ) {
		...
		try {
			RecoveryPlan recoveryPlan = recoveryPlanService.find(id);
			...
			for (RecoveryRecord	recoveryRecord : recoveryPlan.getRecoveryRecordList()) {
				...
				recoveryRecord_map.put("effect", recoveryRecord.getEffect());//康护内容
