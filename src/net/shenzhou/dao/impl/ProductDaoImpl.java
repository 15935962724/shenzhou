...
//		}
	...
	public List<Object[]> findSalesList(Date beginDate, Date endDate, Integer count) {
		...
		Join<Product, net.shenzhou.entity.Order> order = orderItems.join("order");
...
//		}
	...
	public Map<String ,Object> findList(String  file){
		...
		List<Product> list = super.findList(criteriaQuery, null, null, null, null);
			...
			//总页数
			Integer pagecount = (list.size()+pageSize-1)/pageSize;
			...
			if (list.size()>0){
				if(pageNumber>=pagecount){
					product_list = list.subList((pagenumber-1)*pageSize, list.size());
				}else{
					product_list = list.subList((pagenumber-1)*pageSize, pageSize*pagenumber);
				}
				if (pageNumber>pagecount) {
					 status = "500";
					 message = "无更多数据";
			...
			for(Product product : product_list){
				...
				data_map.put("id", product.getId());
				data_map.put("name", product.getName());
				data_map.put("price", product.getPrice());
				data_map.put("image",product.getImage());
				data_map.put("image",product.getImage());
				data_map.put("stock",product.getStock());
				data_map.put("cost",product.getCost());
				data_map.put("image", product.getImage());
				pro_list.add(data_map);
			}
			product_map.put("product",pro_list);	
