 ...
 *
 */
@Service("mechanismImageServiceImpl")
public class MechanismImageServiceImpl implements MechanismImageService, ServletContextAware {
...
//								(打水印去掉)ImageUtils.addWatermark(mediumTempFile, mediumTempFile, watermarkFile, setting.getWatermarkPosition(), setting.getWatermarkAlpha());
	...
	public void build(MechanismImage mechanismImage) {
		MultipartFile multipartFile = mechanismImage.getFile();
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						mechanismImage.setSource(storagePlugin.getUrl(sourcePath));
						mechanismImage.setLarge(storagePlugin.getUrl(largePath));
						mechanismImage.setMedium(storagePlugin.getUrl(mediumPath));
						mechanismImage.setThumbnail(storagePlugin.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
