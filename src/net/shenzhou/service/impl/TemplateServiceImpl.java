...
import net.shenzhou.Template.Type;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/shenzhou/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(shenzhouXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/shenzhou/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File shenzhouXmlFile = new ClassPathResource(CommonAttributes.shenzhou_XML_PATH).getFile();
			Document document = new SAXReader().read(shenzhouXmlFile);
			Element element = (Element) document.selectSingleNode("/shenzhou/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
