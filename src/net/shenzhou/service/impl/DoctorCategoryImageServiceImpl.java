...
import net.shenzhou.service.DoctorCategoryImageService;
...
@Service("doctorCategoryImageServiceImpl")
public class DoctorCategoryImageServiceImpl implements DoctorCategoryImageService, ServletContextAware {
	...
	public void build(DoctorCategoryImage doctorCategoryImage) {
		MultipartFile multipartFile = doctorCategoryImage.getFile();
...
//				String uploadPath = FreemarkerUtils.process(setting.getImageUploadPath(), model);
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						doctorCategoryImage.setSource(storagePlugin.getUrl(sourcePath));
						doctorCategoryImage.setLarge(storagePlugin.getUrl(largePath));
						doctorCategoryImage.setMedium(storagePlugin.getUrl(mediumPath));
						doctorCategoryImage.setThumbnail(storagePlugin.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
