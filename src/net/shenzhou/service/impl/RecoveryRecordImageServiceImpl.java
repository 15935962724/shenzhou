 ...
 *
 */
@Service("recoveryRecordImageServiceImpl")
public class RecoveryRecordImageServiceImpl implements RecoveryRecordImageService, ServletContextAware {
...
//								(打水印去掉)ImageUtils.addWatermark(mediumTempFile, mediumTempFile, watermarkFile, setting.getWatermarkPosition(), setting.getWatermarkAlpha());
	...
	public void build(RecoveryRecordImage recoveryRecordImage) {
		MultipartFile multipartFile = recoveryRecordImage.getFile();
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						recoveryRecordImage.setSource(storagePlugin.getUrl(sourcePath));
						recoveryRecordImage.setLarge(storagePlugin.getUrl(largePath));
						recoveryRecordImage.setMedium(storagePlugin.getUrl(mediumPath));
						recoveryRecordImage.setThumbnail(storagePlugin.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
