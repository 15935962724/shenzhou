...
public class FileServiceImpl implements FileService, ServletContextAware {
	...
	public String uploadImg(FileType fileType, MultipartFile multipartFile,String paths,String name, boolean async) {
		if (multipartFile == null) {
			return null;
		}
		Setting setting = SettingUtils.get();
...
//			uploadPath = setting.getImageUploadPath();
		...
		try {
			...
			String path = FreemarkerUtils.process(uploadPath, model);
			String destPath = path + name + "." + FilenameUtils.getExtension(multipartFile.getOriginalFilename());
            System.out.println(pluginService.getStoragePlugins(true).size());
			for (StoragePlugin storagePlugin : pluginService.getStoragePlugins(true)) {
				File tempFile = new File(System.getProperty("java.io.tmpdir") + "/upload_" + UUID.randomUUID() + ".tmp");
				if (!tempFile.getParentFile().exists()) {
					tempFile.getParentFile().mkdirs();
