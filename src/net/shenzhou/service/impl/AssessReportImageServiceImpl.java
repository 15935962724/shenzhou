 ...
 *
 */
@Service("assessReportImageServiceImpl")
public class AssessReportImageServiceImpl implements AssessReportImageService, ServletContextAware {
...
//								(打水印去掉)ImageUtils.addWatermark(mediumTempFile, mediumTempFile, watermarkFile, setting.getWatermarkPosition(), setting.getWatermarkAlpha());
	...
	public void build(AssessReportImage  assessReportImage) {
		MultipartFile multipartFile = assessReportImage.getFile();
			...
			try {
				...
				for (StoragePlugin storagePlugin : storagePlugins) {
					if (storagePlugin.getIsEnabled()) {
						...
						assessReportImage.setSource(storagePlugin.getUrl(sourcePath));
						assessReportImage.setLarge(storagePlugin.getUrl(largePath));
						assessReportImage.setMedium(storagePlugin.getUrl(mediumPath));
						assessReportImage.setThumbnail(storagePlugin.getUrl(thumbnailPath));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
