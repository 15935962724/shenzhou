...
import net.shenzhou.Template;
import net.shenzhou.dao.ArticleDao;
...
import net.shenzhou.service.TemplateService;
...
public class StaticServiceImpl implements StaticService, ServletContextAware {
	...
	@Override
	@Transactional(readOnly = true)
	public int build(Journalism journalism) {
			Assert.notNull(journalism);
			delete(journalism);
			Template template = templateService.get("journalismContent");
			int buildCount = 0;
			if (journalism.getIsPublication()) {
				Map<String, Object> model = new HashMap<String, Object>();
				model.put("journalism", journalism);
				for (int pageNumber = 1; pageNumber <= journalism.getTotalPages(); pageNumber++) {
					journalism.setPageNumber(pageNumber);
					buildCount += build(template.getTemplatePath(), journalism.getPath(), model);
				}
	...
	public int buildAll() {
		...
		for (int i = 0; i < journalismDao.count(); i += 20) {
			List<Journalism> journalisms = journalismDao.findList(i, 20, null, null);
			for (Journalism journalism : journalisms) {
				buildCount += build(journalism);
			}
			journalismDao.clear();
		}
	...
	@Override
	@Transactional(readOnly = true)
	public int delete(Journalism journalism) {
			Assert.notNull(journalism);
			int deleteCount = 0;
			for (int pageNumber = 1; pageNumber <= journalism.getTotalPages() + 1000; pageNumber++) {
				journalism.setPageNumber(pageNumber);
				int count = delete(journalism.getPath());
				if (count < 1) {
					break;
				}
				deleteCount += count;
